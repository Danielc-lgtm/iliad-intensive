...
- Behavioral characterization: an optimizer steers the world into a narrow target region; looks only at what the whole system does, not its internals or who is watching
- Control-theory framing; the thermostat as the standard picture; the target as a *convergent attractor* (pulled in from far away, returns after being pushed off)
- The two load-bearing features: a broad range of initial conditions (many starts funnel to the same place); robustness to perturbation (a coasting satellite has no attractor and optimizes nothing; a ball settling in a valley against friction is a weak but genuine optimizer)
- Two deliberate omissions, which the rest of the document depends on: no optimizer/optimized split (the whole system is the object); no goals/preferences/representations (thermostat, feedback circuit, organism, optimization algorithm, house-builders are all optimizers in the same sense -- a structural fact about dynamics, not a mental attribute)
...
- Optimizer case, two changes both in your favour: (1) the initial condition is no longer needed -- a broad basin funnels every start to $T$, so uncertainty in $X$ does not propagate to $Y$; (2) a tiny amount of information predicts the outcome -- learn the proposition $O$ ("an optimizer with target $T$ is operating here"); the future bits gained are $I(O;Y) = H_0 - H(Y\mid O)$ (large), the present bits spent are the description length of $O$ (small, the cost of naming $T$); the *ratio* is the point -- many future bits per present bit, the exact inverse of the no-optimizer case
- Thermostat illustration: measure every air molecule and integrate (astronomically many bits, hopeless under chaos) vs learn "a thermostat set to $21^\circ$C is running" (a handful of bits, confident prediction, no molecule measured)
