...
\frame{\frametitle{Robust Concepts (``True Names'')}
	\begin{block}{Why we need robust concepts}
		{\tiny
		\begin{itemize}
			\item To even state a safety property, we need precise concepts for the things we care about: what the agent is optimising, what its goals are, what it believes about the world
			\item These concepts must stay meaningful under extreme optimisation pressure and self-modification. A concept that quietly breaks down when an agent becomes much more capable is of little use as the basis for a guarantee
			\item Agent foundations looks for such robust concepts, sometimes called \textit{``true names''}: characterisations of agency-related notions sharp enough that we can rely on them at high capability
		\end{itemize}
		}
	\end{block}
    \begin{block}{``True names'' we would like to pin down}
		{\tiny
		\begin{itemize}
...
\frame{\frametitle{Two Pathways of Impact: Modelling vs.\ Implementation}
	\begin{block}{Modelling}
		{\tiny
		\begin{itemize}
			\item Build an \textit{abstract model} of a highly capable agent and use it to make safety claims -- e.g.\ AIXI, reflective oracles
			\item The model need not be runnable. Its value is that it lets us reason about how such agents would behave, and in particular it can show \textit{why} a given alignment proposal would fail
		\end{itemize}
		}
	\end{block}
    \begin{block}{Implementation}
		{\tiny
		\begin{itemize}
			\item Develop a theory to the point where it could be turned into an actual algorithm or running system -- the stated aim of work routed through, e.g., UDT or infra-Bayesian physicalism
			\item On this pathway, researchers often favour \textit{modular} architectures: a cleanly factored world model, a planning/search module, and an explicit representation of the goal, each of which can be inspected and controlled separately (in the spirit of designs like JEPA), rather than one opaque network
			\item A clean factorisation is what would let us point at ``the goal'' or ``the world model'' and check or edit it -- which is exactly why we want the \textit{true names} above
		\end{itemize}
		}
	\end{block}
...
\frame{\frametitle{Dualistic agents}
    ...
    \end{figure}
        ...
        \item Standard models of agency (EU maximisation, AIXI, most model-based RL) operate under this dualistic, ``large agent'' picture
...
\frame{\frametitle{Embedded agents}
	\begin{block}{Why embeddedness is hard}
		...
		\begin{itemize}
			...
			\item Dualistic models struggle to represent this. For example, ``AIXI does not believe the universe contains AIXI agents'': it models the environment as something simpler than itself, so it cannot place a copy of itself inside the environment
