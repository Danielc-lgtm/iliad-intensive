...
Agent foundations looks for \emph{robust concepts}, sometimes called ``true names'', for notions like optimization, goals, world models, and embeddedness. A robust concept is one that does not break down under extreme optimization pressure and stays meaningful for agents far more capable than any we have built. These notes pursue the true name of \emph{optimization}. We take the descriptive route. Instead of starting inside an ideal rational agent (beliefs, preferences, expected utility), we start from the physical world and ask one question: what kind of process can reliably steer the world into a narrow target, and what does physics charge it for doing so?

Thermodynamics answers the second half of that question, for two reasons. We state both now, because the rest of the document is their unpacking.

\textbf{Optimization is local entropy reduction.} An optimizer concentrates probability mass: of the many configurations the world could be in, it funnels a broad set into a few. Concentrating probability mass is the same act as reducing uncertainty, and reducing uncertainty is, as we make precise in Section~\ref{sec:prelim}, reducing entropy. This is not a metaphor. It places the entire machinery of entropy (the second law, fluctuation theorems, the cost of erasing information) directly in charge of optimization, and the modern form of that machinery, ``stochastic thermodynamics'', holds arbitrarily far from equilibrium. So physics constrains the shape of any optimizer actually built in the world.

\textbf{Embedded agents pay for knowledge in physical currency.} Picture two kinds of agent. A \emph{dualistic} agent sits outside the world it acts on, like a player at a console: its inputs and outputs are clean, and ``what it knows'' is a free parameter, a distribution we write down by hand. An \emph{embedded} agent is a part of the world it acts on, so its knowledge cannot be free: it has to be stored somewhere physical, in a brain or a memory register made of the same atoms as everything else. A central conclusion of these notes is that, once entropy is defined correctly (algorithmically, as a property of an individual state rather than of a subjective ensemble), an agent's knowledge of its environment becomes an objective physical quantity, the mutual information between its memory and the environment, and that quantity is exactly the budget it can spend on optimization.

A word on what is new here. For readers with a physics background, the value is mostly translation, not new theorems: recasting familiar thermodynamic facts in information-theoretic language ties them to agent-foundations concepts like world models, optimization power, and the limits on embedded agents. For readers without a physics background, the notes are self-contained. Every thermodynamic notion is defined here in information-theoretic terms, no statistical mechanics is assumed, and the only prerequisite is comfort with elementary discrete probability.
...
Two things need to be kept apart. There is the \emph{system being optimized}: some part of the world whose configuration we care about and which, left alone, could end up many different ways. And there is the \emph{optimizer}: a separate physical thing whose presence drives that system reliably toward a narrow target. The optimizer is not the target, and it is not the funnelling itself; it is the \emph{cause} of the funnelling. The cleanest way to hold the idea is this: an optimizer is a physical thing such that, once you know it is there, your uncertainty about where the optimized system ends up becomes small.

A courier delivering a package to a fixed address makes the two roles concrete. The system being optimized is the package, whose location could be almost anywhere; the optimizer is the courier. Start the package's journey from anywhere in the city, throw up road closures and traffic, and it still arrives, because the courier finds a route around each obstacle. Left to itself the package would go nowhere in particular; the courier is what drives it to the address. In the language of dynamical systems, the courier makes the address a \emph{convergent attractor} for the package: a configuration the package is pulled toward from far off and returns to after being knocked aside. The attractor is a feature of the package's behavior, and the courier is what creates it.

Two features carry all the weight. The first is a \emph{broad range of initial conditions}: the optimizer does not need the system to start anywhere special, since a wide variety of starting points are all driven to the same place. Someone who knows only that the package ``started somewhere in the city'' is highly uncertain about where it began, yet, once told a courier is delivering it, is confident about where it ends up. The second is \emph{robustness to perturbation}: the convergence survives disturbances applied while the process is underway, and this is what separates a genuine optimizer from a coincidence. A satellite coasting in orbit has nothing steering it; nudge it and it follows a new path forever, with no destination it returns to. The courier, blocked by a closed road, takes another and still arrives. Whether something is present to supply that restoring tendency is the whole difference.
...
Model the configuration of the optimized system as a random variable: $X$ for where it starts and $Y$ for where it ends. Before any optimizer is accounted for, $X$ ranges over a broad set, so an observer who knows only that the system starts somewhere in that set has high uncertainty $H(X)$. Now bring in the optimizer and condition on its presence. Because it drives the system into the narrow target whatever the system started from, the observer's uncertainty about the final configuration, $H(Y)$, is small. The gap between the two is the \emph{entropy reduction}
\[
\Delta H \;:=\; H(X) - H(Y),
\]
the number of bits by which the optimizer has narrowed the system's possibilities. A broad range of initial conditions makes $H(X)$ large; robustness is what lets $H(X)$ be large while the convergence still operates; a narrow target makes $H(Y)$ small. Reliably reaching a target that the system's own unaided dynamics would essentially never hit is what it means for $\Delta H$ to be large, and it is the precise form of the opening slogan: a powerful optimizer reliably produces outcomes that would be extremely unlikely to arise under any random process.
...
We can now give a reason to care about optimizers that presupposes nothing about anyone's goals. Suppose you are an agent who wants only to \emph{predict} the future configuration of some part of the world. The claim is that, of everything in the world, optimizers are among the most economical things to know about.

Begin with the ordinary case, where no optimizer is involved. To predict the final state $Y$ of a physical system you must, in general, know its initial state $X$ in detail and push it through the dynamics. This is expensive on two counts. It costs roughly $H(X)$ bits to specify the initial condition, and for a system of any size $H(X)$ is enormous. And the prediction degrades over time rather than improving: chaotic dynamics amplify any error in your knowledge of $X$, so a coarse measurement of $X$ buys you less and less about $Y$ the further ahead you look. Prediction here means paying a large amount of present information for a forecast that gets worse with time.

Now suppose an optimizer is acting on the system, driving it toward a target set $T$. Two things change at once, and both cut in your favour.

First, \emph{you no longer need the initial condition}. The optimizer drives a broad range of initial states all to $T$, so your uncertainty about $X$ simply does not propagate into uncertainty about $Y$. The $H(X)$ bits you would otherwise have had to acquire are now irrelevant to the forecast.

Second, \emph{naming the target is the forecast}. The laborious way to predict where the system ends up is to pin down its exact starting configuration and follow the dynamics forward, step by step. For anything large this is hopeless: the starting configuration is enormous to write down, and chaos means the slightest error in it grows until the forecast is worthless. The optimizer hands you a shortcut. Because it drives the system to the target wherever things began, you can ignore the starting configuration altogether and simply say where the system ends up: at the target. A target is a small thing to know and a sturdy one, since the optimizer reaches it in spite of disturbances you could never have followed.
...
The courier makes the two routes concrete. To predict where the package will end up, you could try to track the courier's vehicle, every traffic light, and every other car on the road, integrating it all forward: a hopeless tangle of contingencies, and chaotic besides, since one missed light reshuffles every turn that follows. Or you could learn a single fact, that the courier is delivering to 14 Elm Street, and predict that the package ends at 14 Elm Street, having modelled none of the traffic. The second route spends almost nothing and predicts almost perfectly.

Robustness sharpens the conclusion. Because the optimizer reaches $T$ in spite of perturbations, your forecast ``the system ends in $T$'' holds even under disturbances you could not have foreseen or measured. You are therefore spared not only the cost of the initial condition but also the cost of modelling the perturbations: the optimizer absorbs both. An agent that cares only about prediction should, for this reason alone, scan its environment for optimizers and model them as optimizers, tracking what they steer toward, because nothing else compresses the future into so short a description.
...
\paragraph{The objection, stated at its strongest.} There is a well-known view that intelligence, and optimization with it, is \emph{observer-relative}: a system counts as intelligent only because it does well on tasks \emph{we} happen to care about, judged by standards \emph{we} happen to hold, and on a different choice of tasks the same system would look unremarkable. The view has a formal backbone in the No Free Lunch theorems, which show that, averaged over all possible problems, no optimizer outperforms blind search; competence is therefore never absolute but always relative to a restricted class of problems, and the choice of that class reflects the observer's interests. It has a philosophical sibling in Dennett's intentional stance, on which calling a system an agent with goals is not a discovery about the system but a \emph{stance} an observer adopts because it pays off predictively, so that whether something ``has goals'' depends on the modelling capabilities and convenience of the observer. On either telling, ``$X$ is an optimizer'' looks less like a fact about $X$ than like a relation between $X$ and a particular onlooker.

\paragraph{Why the thermodynamic picture answers it.} Notice first where we agree with the intentional stance: we too ground the significance of optimizers in their predictive value (Section~\ref{sec:prediction}). The disagreement is over whether that value is relative to a particular observer. It is not. ``Is something here driving this system into a narrow target?'' (a broad set of initial conditions funnelled robustly into a narrow target) is a question about the system and the things acting on it, with an answer that does not mention anyone's preferences. And the predictive leverage a known optimizer provides, a short and accurate description of where the system is headed, is the same for every predictor regardless of what, if anything, that predictor wants. So one need not fix any tasks or standards to single out optimizers: the bare aim of prediction, which any embedded agent has reason to pursue whatever its goals, already picks them out. The reason to care is observer-\emph{independent}.

This does not contradict No Free Lunch, which concerns average competence across \emph{all} problems and says nothing about whether some physical thing is driving a given system into a narrow target. Nor does it deny that an optimizer has a target: which target it is is itself a property of the system (where its dynamics actually funnel), not a verdict imposed from outside. The observer-relativity the objection worries about, namely whose standards judge success, never enters, because we are not grading the optimizer against our standards; we are noting that it has an attractor and using that fact, as any predictor could, to predict the world. Optimization is something the world does, not something an observer projects onto it.

To summarize the conceptual frame before turning to physics: an optimizer is a physical thing that drives the system it acts on into a convergent attractor (Section~\ref{sec:attractor}); the amount of optimization is the entropy it removes from that system (Section~\ref{sec:optentropy}); and any predictor, whatever its goals, has an objective reason to track optimizers (Sections~\ref{sec:prediction}--\ref{sec:objective}). What remains is to ask what physics permits. Optimization is entropy reduction in a subsystem, that is, \emph{local} entropy reduction, and the next section shows that \emph{global} entropy reduction is physically impossible. That tension, and the bookkeeping that resolves it, organizes the rest of these notes.
