...
\subsection{Optimizers as convergent attractors}
\label{sec:attractor}

The characterization these notes build on looks only at what a system \emph{does}, not at what is inside it or who is watching. An \emph{optimizer} is anything that steers the world into a narrow region of its configuration space: out of the many configurations the relevant part of the world might be in, the optimizer drives it toward a small \emph{target set} and holds it there.

The cleanest intuitions come from control theory, but the phenomenon is general, so take a concrete case: a courier delivering a package to a fixed address. Set the courier down anywhere in the city, throw up road closures and traffic, and the package still arrives, because the courier finds a new route around each obstacle and converges on the destination. The set of configurations in which the package sits at its address is a \emph{convergent attractor}: a region the process is driven into from far away and returns to after being pushed off course.

Two features carry all the weight, and both are visible in the courier.

The first is a \emph{broad range of initial conditions}. The optimizer does not need to begin from one special configuration; a wide variety of starting points are all funnelled to the same place. Someone who knows only that the courier ``started somewhere in the city'' is highly uncertain about the initial position, yet can be confident about where the package ends up.

The second is \emph{robustness to perturbation}. The convergence survives disturbances applied while the process is underway, and this is what separates genuine optimization from a coincidence. A satellite coasting in orbit traces a precise path, but nudge it and it follows a different path forever: there is no destination it reliably returns to, so it optimizes nothing. The courier, blocked by a closed road, takes another and still arrives. The presence or absence of this restoring tendency is the whole difference.

One omission is worth flagging, because the rest of the document depends on it: the characterization says nothing about goals, preferences, or representations stored in a mind. A courier, a feedback circuit, a chess engine driving a game toward checkmate, a growing organism, a running optimization algorithm, and a team building a house are optimizers in exactly the same sense, each a physical process that drives a broad set of possibilities into a narrow target and defends it against perturbation. What unifies them is a structural fact about their dynamics, not a mental attribute. The next three subsections turn this behavioral picture into a number, explain why even an observer with no stake in the target should care about it, and argue that optimizer-hood is therefore an objective feature of the world.
...
Model the configuration of the relevant subsystem of the world as a random variable: $X$ for the initial configuration and $Y$ for the final one. The randomness in $X$ encodes the breadth of the set of configurations the system might start in: there are many of them, so an observer who knows only that the system starts somewhere in this set has high uncertainty $H(X)$. An optimizer funnels this broad set into a narrow target, so the same observer's uncertainty about the final configuration, $H(Y)$, is small. The natural measure of how much funnelling occurred is the \emph{entropy reduction}
\[
\Delta H \;:=\; H(X) - H(Y),
\]
the number of bits by which the optimizer has narrowed the world's possibilities. A broad range of initial conditions makes $H(X)$ large; robustness is what lets $H(X)$ be large while the convergence still operates; a narrow target makes $H(Y)$ small. Reliably reaching a target that random dynamics would essentially never hit is exactly what it means for $\Delta H$ to be large, and it is the precise form of the opening slogan: a powerful optimizer reliably produces outcomes that would be extremely unlikely to arise under any random process.
...
We can now give a reason to care about optimizers that presupposes nothing about anyone's goals. Suppose you are an agent who wants only to \emph{predict} the future configuration of some part of the world. The claim is that optimizers are, in a precise information-theoretic sense, the most economical things in the world to know about.
...
First, \emph{you no longer need the initial condition}. The defining property of the optimizer is that a broad range of initial states all funnel to $T$, so your uncertainty about $X$ simply does not propagate into uncertainty about $Y$. The $H(X)$ bits you would have had to acquire are now irrelevant to the forecast.

Second, \emph{a short description predicts the outcome}. To forecast where the process ends up, all you need is the optimizer's target $T$: then you predict $Y \in T$. Compare the two models you could use to predict that same outcome. The \emph{initial-condition model} specifies $X$ and integrates it forward; it costs about $H(X)$ bits, and it is fragile, because chaos amplifies any error in $X$. The \emph{target model} specifies $T$ and predicts $Y \in T$; it costs only the length of the target's description, and it is robust, because the optimizer reaches $T$ in spite of perturbations. A narrow target is a tiny object next to a full initial microstate, so its description is far shorter than $H(X)$. The optimizer thus lets you trade a huge, fragile description of where the system started for a small, robust description of where it is going, and for the macroscopic outcome you actually care about, the small description is the more reliable of the two.

No free lunch is hidden in this. Learning the target removes no more uncertainty about $Y$ than the bits used to name the target; no observation can tell you more about a variable than its own information content. What makes optimizers worth singling out is comparative, and it is about \emph{cost}: the price of the target model, the size of the target, is minute next to $H(X)$, the price of the only other route to the same forecast.
...
\paragraph{Why the thermodynamic picture answers it.} Notice first where we agree with the intentional stance: we too ground the significance of optimizers in their predictive value (Section~\ref{sec:prediction}). The disagreement is over whether that value is relative to a particular observer. It is not. ``Is there a convergent attractor here?'' (a broad set of initial conditions funnelled robustly into a narrow target) is a question about the system's own dynamics, with an answer that does not mention anyone's preferences. And the predictive leverage an attractor provides, a short and accurate description of the outcome, is the same for every predictor regardless of what, if anything, that predictor wants. So one need not fix any tasks or standards to single out optimizers: the bare aim of prediction, which any embedded agent has reason to pursue whatever its goals, already picks them out. The reason to care is observer-\emph{independent}.

This does not contradict No Free Lunch, which concerns average competence across \emph{all} problems and says nothing about whether a given physical system contains a convergent attractor. Nor does it deny that an optimizer has a target: which target it is is itself a property of the system (where its dynamics actually funnel), not a verdict imposed from outside. The observer-relativity the objection worries about, namely whose standards judge success, never enters, because we are not grading the optimizer against our standards; we are noting that it has an attractor and using that fact, as any predictor could, to predict the world. Optimization is something the world does, not something an observer projects onto it.

To summarize the conceptual frame before turning to physics: an optimizer is a convergent attractor (Section~\ref{sec:attractor}); the amount of optimization is the entropy it removes (Section~\ref{sec:optentropy}); and any predictor, whatever its goals, has an objective reason to track optimizers (Sections~\ref{sec:prediction}--\ref{sec:objective}). What remains is to ask what physics permits. Optimization is entropy reduction in a subsystem, that is, \emph{local} entropy reduction, and the next section shows that \emph{global} entropy reduction is physically impossible. That tension, and the bookkeeping that resolves it, organizes the rest of these notes.
...
Return to Section~\ref{sec:optsys}. An optimizer funnels a broad range of configurations into a narrow target set. What does the existence of such a system imply, thermodynamically?
...
\begin{itemize}[leftmargin=2em]
  \item Optimization, characterized behaviorally, is the reliable steering of a system from a broad range of initial conditions into a narrow target set, robustly to perturbation: a convergent attractor. Its quantitative measure is entropy reduction, the bits by which the process narrows the world's possibilities. No agent/environment split is needed to define it, and no goal needs to be mentioned to price it.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Daniel~C and A.~Ebtekar, \emph{Algorithmic thermodynamics and three types of optimization}, AI Alignment Forum, 2025. The central organizing source for these notes: the characterization of optimizers as convergent attractors and the information-theoretic case for attending to them whatever one's goals (Section~\ref{sec:optsys}), the three types of optimization (Section~\ref{sec:threetypes}), the argument that entropy should objectively measure optimization capacity (Section~\ref{sec:subjective}), the algorithmic refinements of the three types, and the embedded-agency synthesis (Section~\ref{sec:embedded}).
