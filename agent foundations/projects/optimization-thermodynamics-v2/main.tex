...
{\large From convergent attractors to algorithmic entropy, and what physics charges an embedded agent for steering the world} \\[1ex]
...
Agent foundations is the search for \emph{robust concepts} (sometimes called ``true names'') for things like optimization, goals, world models, and embeddedness. A robust concept is one that keeps its meaning under extreme optimization pressure, and for agents far more capable than any we have built. This document goes after the true name of \emph{optimization}, and it takes the descriptive route. Rather than start inside an idealized rational agent, with its beliefs and preferences and expected utility, we start from the physical world and ask one question: what kind of process can reliably steer the world into a narrow target, and what does physics charge it for doing so?

Thermodynamics answers the second half of that question, for two reasons. Here they are up front; the rest of the document unpacks them.

\textbf{Optimization is local entropy reduction.} An optimizer concentrates probability mass: out of the many configurations the world could be in, it funnels a broad set down to a few. But concentrating probability mass just is reducing uncertainty, and reducing uncertainty just is reducing entropy (Section~\ref{sec:prelim} makes this exact). This is not an analogy. It puts the full machinery of entropy (the second law, fluctuation theorems, the cost of erasing a bit) directly in charge of optimization, and the modern version of that machinery, \emph{stochastic thermodynamics}, holds arbitrarily far from equilibrium. So physics constrains the shape of any optimizer you could actually build.

\textbf{Embedded agents pay for knowledge in physical currency.} Picture two kinds of agent. A \emph{dualistic} agent sits outside the world it acts on, like a player at a console: clean inputs, clean outputs, and ``what it knows'' a free parameter, a distribution we write down by hand. An \emph{embedded} agent is part of the world it acts on, so its knowledge is not free. It has to be stored somewhere physical, in a brain or a memory register made of the same atoms as everything else. One of the main conclusions of these notes is that once you define entropy correctly (algorithmically, as a property of an individual state rather than of a subjective ensemble), an agent's knowledge of its environment turns into an objective physical quantity, the mutual information between its memory and the environment, and that quantity is exactly the budget it has to spend on optimization.

A word on what is new here, and for whom. If you have a physics background, the value is mostly translation rather than new theorems: recasting familiar thermodynamic facts in information-theoretic language ties them to agent-foundations notions like world models, optimization power, and the limits on embedded agents. If you do not, the notes are self-contained. Every thermodynamic idea is defined here in information-theoretic terms, no statistical mechanics is assumed, and the only prerequisite is comfort with elementary discrete probability.

\paragraph{Plan of the notes.} Section~\ref{sec:prelim} builds the probabilistic and information-theoretic toolkit from scratch: entropy, mutual information, divergence, and the coding interpretations that make these quantities meaningful. Section~\ref{sec:optsys} asks what optimization is: it characterizes an optimizer behaviorally, as a physical thing that drives the system it acts on, from a broad range of initial conditions and robustly to perturbation, into a narrow target (a convergent attractor), translates this into entropy reduction, and argues that any observer who cares about prediction has an objective, information-theoretic reason to attend to optimizers, whoever is watching. Section~\ref{sec:secondlaw} confronts this picture with the reversibility of microscopic physics and derives the second law of thermodynamics, with full proof, in a minimal coarse-grained setting. Section~\ref{sec:threetypes} then classifies the three ways an embedded agent can reduce a subsystem's entropy while respecting global information conservation: dumping it into the environment, absorbing it into memory by measurement, or spending pre-existing mutual information with the subsystem. Section~\ref{sec:subjective} exposes a conceptual problem at the heart of the standard formalism: entropy, as usually defined, depends on a subjective probability distribution supplied from outside the physics. Section~\ref{sec:algo} develops the resolution, algorithmic thermodynamics: Kolmogorov complexity as entropy, the algorithmic second law, and an exact analysis of Maxwell's demon, with the emphasis throughout on how an objective notion of entropy dissolves the subjectivity problem. Section~\ref{sec:embedded} assembles the synthesis for embedded agency: knowledge as an endogenous physical resource. Section~\ref{sec:takeaways} collects the takeaways. Appendix~\ref{sec:coins} develops, in parallel to the main line, a self-contained toy thermodynamics built out of biased coins (Wentworth's generalized heat engine), in which the analogues of heat, work, energy conservation, and the impossibility of perpetual motion can all be verified by hand; the main text points to it wherever a concrete blind-policy example helps. Appendix~\ref{sec:tl} develops the Touchette--Lloyd theorem, which makes the third channel quantitative: the entropy reduction an agent can achieve beyond a blind baseline is bounded by the mutual information between its action and the environment, with fully worked examples.

\section{A self-contained toolkit: probability, entropy, information}
\label{sec:prelim}

This section develops everything we need from information theory. If you already know Shannon entropy and mutual information, skim it, but note the two things we lean on later: the coding interpretation of Section~\ref{sec:coding}, which recurs constantly, and the log-sum inequality (Lemma~\ref{lem:logsum}), which powers our proof of the second law.
...
We will lean on one reading of entropy more than any other: entropy as data compression. Suppose you have to transmit the value of $X$ to a colleague in binary, and you want to send as few bits on average as you can. The trick is to spend short codewords on the likely values and long ones on the unlikely values. Shannon's source coding theorem makes this precise: there is a code (a prefix-free assignment of binary strings to outcomes) whose codeword for outcome $x$ has length essentially $\log \frac{1}{\mu(x)}$ bits, and no code can achieve a smaller average length than the entropy. Therefore
\[
...
The quantity inside the expectation needs its own name, because Section~\ref{sec:subjective} turns on the distinction between it and its average.
...
Notice carefully what depends on what here. The codelength $\hat H(x, \mu)$ is a property of the \emph{pair} (outcome, distribution): the same physical outcome $x$ gets a short codelength under a distribution that expected it and a long one under a distribution that did not. So far there is no such thing as ``the entropy of an individual outcome $x$'' on its own, with no distribution to read it against. Hold onto that; it becomes the crux of Section~\ref{sec:subjective}.
...
The word ``optimization'' already means two different things, and the gap between them is the whole problem. In computer science, an optimization algorithm is a program that outputs the solution, or an approximation of it, to an optimization problem: an objective function to be minimized over some feasible region. A program that returns $x \approx \sqrt 2$, the minimizer of $y = (x^2 - 2)^2$ on the real line, is optimizing in this sense. In operations research and engineering, optimization means something physically heftier: reworking a process or an artifact so it better fits a purpose, like reorganizing a factory to make more nails per unit cost. Optimizing a number inside a computer and optimizing a factory are obviously related, but what \emph{is} the relation? What does a physical process have to do to earn the name optimization?

\subsection{Optimizers and convergent attractors}
\label{sec:attractor}

Two things have to be kept apart. First, the \emph{system being optimized}: some part of the world whose configuration we care about, and which, left alone, could end up all sorts of ways. Second, the \emph{optimizer}: a separate physical thing whose presence drives that system reliably toward a narrow target. The optimizer is not the target, and it is not the funnelling; it is the \emph{cause} of the funnelling. Here is the cleanest way to hold the idea: an optimizer is a physical thing such that, once you know it is there, your uncertainty about where the optimized system ends up collapses.

A courier delivering a package to a fixed address makes the two roles concrete. The package is the system being optimized; its location could be almost anywhere. The courier is the optimizer. Start the package off anywhere in the city, throw road closures and traffic in its way, and it still arrives, because the courier finds a route around each obstacle. Left alone the package goes nowhere in particular; the courier is what drives it to the address. In the language of dynamical systems, the courier makes the address a \emph{convergent attractor} for the package: a configuration it is pulled toward from far away and returns to after being knocked aside. The attractor is a fact about the package's behavior; the courier is what creates it.

Two features carry all the weight. The first is a \emph{broad range of initial conditions}: the optimizer does not need the system to start anywhere special, because lots of different starting points all get driven to the same place. Someone who knows only that the package ``started somewhere in the city'' is very unsure where it began, but tell them a courier is delivering it and they become confident about where it ends up. The second is \emph{robustness to perturbation}: the convergence survives disturbances thrown at it while the process is underway. This is what separates a real optimizer from a coincidence. A satellite coasting in orbit has nothing steering it; nudge it and it drifts onto a new path forever, with no home to return to. The courier, blocked by a closed road, takes a different one and still arrives. The whole difference is whether something is there to supply that restoring tendency.

One omission matters for everything that follows: calling something an optimizer says nothing about goals, preferences, or representations stored in a mind. A courier, a feedback circuit, a chess engine driving a game toward checkmate, a growing organism, a running optimization algorithm, and a team building a house are optimizers in exactly the same sense, each a physical thing whose presence drives some system into a narrow target and holds it there against perturbation. What unifies them is a structural fact about their effect on the world, not a mental attribute. The next three subsections turn this picture into a number, explain why even an observer with no stake in the target should care about it, and argue that optimizer-hood is therefore an objective feature of the world.

\subsection{Optimization as entropy reduction}
\label{sec:optentropy}

The convergent-attractor picture translates directly into the language of Section~\ref{sec:prelim}, and the translation is the bridge to physics.

Model the configuration of the optimized system as a random variable: $X$ for where it starts, $Y$ for where it ends. Before we account for any optimizer, $X$ ranges over a broad set, so someone who knows only that the system starts somewhere in that set has high uncertainty $H(X)$. Now add the optimizer and condition on its presence. It drives the system into the narrow target no matter where the system started, so the uncertainty left about the final configuration, $H(Y)$, is small. The gap between the two is the \emph{entropy reduction}
\[
\Delta H \;:=\; H(X) - H(Y),
\]
the number of bits by which the optimizer has narrowed the system's possibilities. A broad range of initial conditions makes $H(X)$ large; robustness is what keeps $H(X)$ large while the convergence still works; a narrow target makes $H(Y)$ small. A large $\Delta H$ means reliably hitting a target the system's own unaided dynamics would essentially never reach, which is just the opening slogan made precise: a powerful optimizer reliably produces outcomes that would be wildly unlikely under any random process.
...
\subsection{Why a predictor should attend to optimizers}
\label{sec:prediction}

Here is a reason to care about optimizers that assumes nothing about anyone's goals. Suppose all you want to do is \emph{predict} the future configuration of some part of the world. The claim is that, of all the things you could study, optimizers are among the cheapest to know about.

Start with the ordinary case, no optimizer involved. To predict the final state $Y$ of a physical system, you generally have to know its initial state $X$ in detail and push it through the dynamics. That is expensive twice over. Specifying the initial condition costs about $H(X)$ bits, and for a system of any size $H(X)$ is enormous. Worse, the prediction decays as you look further ahead: chaos amplifies any error in your knowledge of $X$, so a coarse measurement of $X$ tells you less and less about $Y$ over time. You pay a fortune in present information for a forecast that only gets worse.

Now suppose an optimizer is acting on the system, driving it toward a target set $T$. Two things change at once, and both are in your favour.

First, \emph{you no longer need the initial condition}. The optimizer drives a broad range of initial states all to $T$, so your uncertainty about $X$ never propagates into uncertainty about $Y$. The $H(X)$ bits you would otherwise have had to acquire are simply irrelevant to the forecast.

Second, \emph{naming the target is the forecast}. The laborious way to predict where the system ends up is to pin down its exact starting configuration and grind the dynamics forward step by step. For anything large that is hopeless: the starting configuration is enormous to write down, and chaos means the slightest error in it blows up until the forecast is worthless. The optimizer hands you a shortcut. Since it drives the system to the target wherever things began, you can throw away the starting configuration and just say where the system ends up: at the target. A target is a small thing to know, and a sturdy one, because the optimizer reaches it despite disturbances you could never have tracked.

This is a saving of effort, not a free lunch. Saying ``it ends up at the target'' \emph{is} the forecast; you have not conjured extra knowledge out of nowhere. The point is only that knowing where a process is headed is cheap and stable, whereas reconstructing where it started, the sole alternative route to the same forecast, is vast and fragile. Optimizers are worth hunting for because they let you predict an outcome you would otherwise have almost no way to reach.

The courier makes the two routes concrete. To predict where the package ends up, you could try to track the courier's vehicle, every traffic light, and every other car on the road, and integrate it all forward: a hopeless tangle of contingencies, chaotic besides, since one missed light reshuffles every turn after it. Or you could learn one fact, that the courier is delivering to 14 Elm Street, and predict that the package ends at 14 Elm Street, having modelled none of the traffic. The second route spends almost nothing and predicts almost perfectly.

Robustness sharpens the conclusion. Because the optimizer reaches $T$ in spite of perturbations, your forecast ``the system ends in $T$'' survives disturbances you could not have foreseen or measured. So you are spared not just the cost of the initial condition but the cost of modelling the perturbations too; the optimizer absorbs both. For this reason alone, an agent that cares only about prediction should scan its environment for optimizers and track what they steer toward, because nothing else packs the future into so short a description.

\subsection{Optimization is an objective feature, not observer-relative}
\label{sec:objective}

The argument of Section~\ref{sec:prediction} answers a standing objection to treating ``optimization'' or ``intelligence'' as a real feature of the world at all.

\paragraph{The objection, stated at its strongest.} A well-known view holds that intelligence, and optimization with it, is \emph{observer-relative}: a system counts as intelligent only because it does well on the tasks \emph{we} happen to care about, by the standards \emph{we} happen to hold, and on a different choice of tasks the same system would look unremarkable. The view has a formal backbone in the No Free Lunch theorems, which show that, averaged over all possible problems, no optimizer beats blind search; competence is then never absolute, always relative to some restricted class of problems, and the choice of class reflects the observer's interests. It has a philosophical sibling in Dennett's intentional stance, on which calling a system an agent with goals is not a discovery about the system but a \emph{stance} an observer takes up because it pays off predictively, so that whether something ``has goals'' depends on the observer's modelling powers and convenience. Either way, ``$X$ is an optimizer'' looks less like a fact about $X$ than like a relation between $X$ and some particular onlooker.

\paragraph{Why the thermodynamic picture answers it.} Notice first where we agree with the intentional stance: we too ground the significance of optimizers in their predictive value (Section~\ref{sec:prediction}). The disagreement is over whether that value is relative to a particular observer, and it is not. ``Is something here driving this system into a narrow target, from a broad set of initial conditions and robustly to perturbation?'' is a question about the system and the things acting on it, and its answer never mentions anyone's preferences. The predictive leverage a known optimizer hands you, a short and accurate description of where the system is headed, is the same for every predictor no matter what, if anything, it wants. So you do not have to fix any tasks or standards to pick out optimizers: the bare aim of prediction, which any embedded agent has reason to pursue whatever its goals, already picks them out. The reason to care is observer-\emph{independent}.

None of this contradicts No Free Lunch, which is about average competence across \emph{all} problems and says nothing about whether some physical thing is driving a given system into a narrow target. Nor does it deny that an optimizer has a target: which target is itself a property of the system (where its dynamics actually funnel), not a verdict imposed from outside. The observer-relativity the objection worries about, whose standards get to judge success, never enters, because we are not grading the optimizer against our standards at all; we are noting that it has an attractor and using that fact, as any predictor could, to predict the world. Optimization is something the world does, not something an observer paints onto it.

To collect the conceptual frame before we turn to physics: an optimizer is a physical thing that drives the system it acts on into a convergent attractor (Section~\ref{sec:attractor}); the amount of optimization is the entropy it removes from that system (Section~\ref{sec:optentropy}); and any predictor, whatever its goals, has an objective reason to track optimizers (Sections~\ref{sec:prediction}--\ref{sec:objective}). What is left is to ask what physics permits. Optimization is entropy reduction in a subsystem, that is, \emph{local} entropy reduction, and the next section shows that \emph{global} entropy reduction is physically impossible. That tension, and the bookkeeping that resolves it, organizes the rest of these notes.
...
At the most fundamental level we can describe, the world does not forget. Take classical mechanics. The complete microscopic state of a system, its \emph{microstate}, is the list of every particle's position and momentum: a single point in the system's \emph{phase space}. The laws of motion carry each microstate along a unique trajectory, fixing the future from the present and, equally, the past from the present. So two distinct microstates can never flow into one: if they did, running the laws backward from the shared future state could not tell which to return to, and the laws permit no such ambiguity. Over any fixed interval, then, the evolution of an isolated system is a \emph{bijection} on microstates, a perfect invertible pairing of starts with ends. Liouville's theorem adds more: the evolution preserves not just distinctness but phase-space \emph{volume}, so a blob of microstates can be stretched and folded without limit and yet keep its total volume. Quantum mechanics says the same in other notation: isolated evolution is unitary, hence invertible and volume-preserving. Either way, microscopic information is never created and never destroyed.

\subsection{Why the funneling picture cannot hold globally}
\label{sec:nofunnel}

Now hold the convergent-attractor picture of optimization up against this reversibility. Taken literally at the microscopic level, the two flatly conflict. Funneling means many initial states end up in the same few target states: a many-to-one map. Reversibility says the microscopic dynamics are one-to-one. A perfect funnel is microscopically impossible.

There is a second, closely related obstruction: the second law of thermodynamics itself, that the entropy of an isolated system tends not to decrease. In the framework we are about to build, the second law is not an extra postulate but a \emph{consequence} of reversibility, and seeing why illuminates both. The way out of the apparent paradox (optimization plainly happens, yet physics forbids funneling) is the central bookkeeping principle of these notes:
...
The remainder of this section makes the first sentence precise and proves it. Section~\ref{sec:threetypes} and Appendix~\ref{sec:tl} take up the second sentence: where the squeezed-out information can go, and how much squeezing an agent can do.

\subsection{Coarse-graining: where probability enters a deterministic world}
\label{sec:coarse}

If the microscopic dynamics are deterministic and information-preserving, where do probability and rising entropy come from? From the fact that we never track the microstate. A gram of matter has on the order of $10^{23}$ coordinates, each to unbounded precision; no observer, and no embedded agent, can hold that. What we track instead is a \emph{coarse-grained} description. Partition the phase space into discrete cells, each lumping together all the microstates we cannot tell apart (every particle's position and momentum to finitely many digits, say, or just a few macroscopic variables), and describe the system by naming the cell it occupies.

The dynamics of cells, unlike the dynamics of microstates, are genuinely stochastic. The current cell does not fix the next one, because different microstates inside it flow to different places. The best we can write down is a transition probability $P(y, x)$: the fraction of cell $x$, measured by phase-space volume, that lands in cell $y$ one step later. This randomness is not metaphysical; it is the shadow of the microscopic detail we declined to resolve. Chaotic dynamics keep pulling that detail up across the resolution scale, so the coarse trajectory looks like a string of random jumps even though the flow beneath it is deterministic.

One structural assumption underlies all of stochastic thermodynamics: the coarse trajectory is a \emph{Markov process}, meaning the distribution of the next cell depends only on the current cell and not on the earlier history. Intuitively, the coarse-graining is good enough that the current cell holds everything about the past that matters for the coarse future, and the discarded detail acts like fresh noise at each step rather than hidden memory. Whether a given coarse-graining is really Markovian is a deep and still-open question, but two reassurances help. First, there are exactly solvable models, the \emph{multibaker maps}: deterministic, time-reversible, chaotic systems whose coarse-grainings provably reproduce arbitrary Markov chains, with all the randomness sitting in the initial condition. They show that macroscopic stochasticity and irreversibility are fully compatible with microscopic determinism and reversibility. Second, the Markov property is what makes everyday statistics work, and its \emph{failure backward in time} is the arrow of time. A dropped glass shatters at a predictable moment, and its shards follow well-defined local statistics that owe nothing to events at the neighbor's house. Run the film backward and that breaks: to retrodict when a shattered vase was dropped, local statistics are useless, and your best clue might be a conversation about the accident taking place next door. Forward evolution obeys memoryless local laws; backward evolution does not. That asymmetry is exactly what the Markov property encodes.

One more consequence of reversibility, and then the second law. Liouville's theorem makes phase-space volume a \emph{stationary measure} of the chain: weight each cell by its volume, and one step carries that weighting to itself. We take all cells to have equal volume (a \emph{mesoscopic} coarse-graining, what you get by truncating every coordinate to fixed precision). Stationarity of the uniform weighting then pins down the transition probabilities. With $\pi$ constant, the condition $\sum_x P(y,x)\,\pi(x) = \pi(y)$ cancels to $\sum_x P(y, x) = 1$ for every $y$: the probabilities flowing \emph{into} each cell sum to one, just as those flowing \emph{out of} each cell do. A matrix with both properties is \emph{doubly stochastic}. This is the fingerprint reversibility leaves on the coarse dynamics: a doubly stochastic step can shuffle and spread probability but never concentrate it, since concentration would pack several cells' worth of volume into fewer cells, which Liouville forbids.\footnote{Everything in these notes generalizes to coarse-grainings with unequal cell volumes: one measures entropy \emph{relative to} the stationary volume measure $\pi$, replacing $H(\mu)$ by $H_\pi(\mu) := \sum_x \mu(x) \log \frac{\pi(x)}{\mu(x)}$ and the codelength by $\hat H_\pi(x,\mu) := \log\frac{\pi(x)}{\mu(x)}$. This is standard, but not needed for anything here; equal cells keep the notation light at no conceptual cost.}
...
Notice what the proof used: the list is short, and each item is physical. We used the Markov property, to write one step as a transition matrix, and double stochasticity, which came from Liouville's theorem, that is, from the \emph{reversibility} of the microscopic dynamics. Nothing else. So macroscopic irreversibility, the rise of entropy, is not in tension with microscopic reversibility; it follows from it, once we grant that observers track coarse cells rather than microstates. That is the precise content of the slogan ``the second law follows from the reversibility of physics''.
...
The second channel is subtler: the agent reduces $H(S)$ by \emph{measuring} $S$ and storing the outcomes, so that the subsystem's entropy is transferred into the agent's own memory. This is the operating principle of one of the most famous thought experiments in thermodynamics.
...
Type 3 raises a quantitative question, answered in Appendix~\ref{sec:tl}: if steering is paid for by spending mutual information shared with the subsystem, exactly how many bits of entropy reduction does each bit of mutual information buy? The Touchette--Lloyd theorem, developed there, gives the bound (one bit of entropy reduction beyond the blind baseline per bit of mutual information between action and environment), and we draw on its statement in what follows.

\section{The trouble with subjective entropy}
\label{sec:subjective}

Everything so far has treated the distribution $\mu$, the thing all our entropies are defined against, as simply given. This section pokes at that assumption, and finds that it breaks down right where agent foundations needs it to hold. The critique, and its resolution in the next section, follow Daniel~C and Ebtekar's essay and the foundational paper of Ebtekar and Hutter.

\subsection{Entropy is supposed to measure the capacity to optimize}
\label{sec:capacity}

Start with what entropy is \emph{for}. Across physics and engineering, entropy's job is to measure a system's capacity to do work. Everything so far (Sections~\ref{sec:secondlaw}--\ref{sec:threetypes}, and the coin world of Appendix~\ref{sec:coins}) lets us read ``capacity to do work'' as \emph{capacity to optimize}: negentropy is the fuel that funnels broad distributions into narrow ones.

Apply this to the demon after a long run of Type-2 optimization. Its memory holds, say, $m$ bits, some now filled with measurement records. The demon's \emph{remaining} capacity to optimize the gas is its free memory: the total size minus the entropy already stored. Each further bit of optimization eats another bit of free memory. Here is the question everything turns on: \emph{is the demon's free memory an objective, physical quantity?} It surely ought to be. Whether the hardware can absorb another billion measurements is a fact about the demon's physical configuration, with engineering consequences (how much longer it can refrigerate, how much heat it must eventually pay to clear the records). It would be strange for the demon's actual ability to act on the gas to depend on the beliefs of some outside observer describing it. Yet that is exactly what the standard formalism delivers.

\subsection{The exogenous distribution}
\label{sec:exogenous}

Recall the shape of the standard definitions (Definition~\ref{def:codelength}). The entropy of an \emph{individual} state $x$ is undefined. What is defined is the Shannon codelength $\hat H(x, \mu) = \log \frac{1}{\mu(x)}$ of a state \emph{relative to a distribution} $\mu$, together with its average, the Gibbs--Shannon entropy $H(\mu)$. The distribution $\mu$ is an input, not an output: it encodes somebody's knowledge of the system, and the formalism never says whose knowledge, where it is stored, how it was obtained, or what it cost. The standard framework, in our terms, treats knowledge as \emph{exogenous}.

At the great equilibrium successes of thermodynamics this exogeneity is harmless, because the right $\mu$ is essentially forced. But ``harmless'' rests on three conditions. Call $\mu$ a \emph{useful summary} of an actual physical state $x$ when all three hold:
\begin{enumerate}[leftmargin=2em]
...
Far from equilibrium, and above all for \emph{information-bearing} states, each of the three conditions can fail on its own, and each failure breaks the formalism in its own instructive way.
...
Algorithmic thermodynamics, developed by Ebtekar and Hutter on foundations from Bennett, Zurek, G\'acs, and Levin, keeps the entire coarse-grained Markov framework of Section~\ref{sec:secondlaw} and changes one thing: it replaces the subjective ensemble $\mu$ with universal computation. Instead of asking ``how surprising is $x$ under $\mu$?'', it asks ``how hard is $x$ to describe, full stop?'' The payoff is conceptual: an objective, observer-free entropy that \emph{dissolves the subjectivity problem} of Section~\ref{sec:subjective}, defined for a single physical state with no ensemble in sight. Along the way we get a second law and fluctuation bounds for individual trajectories, and an exact account of Maxwell's demon.
...
The right mental model for $K(x)$ is \emph{optimal lossless compression}: the size of the smallest self-contained representation from which $x$ can be regenerated exactly. A few examples fix the scale. A million zeros has tiny complexity: a fifteen-byte program prints it. The first million digits of $\pi$ have tiny complexity too, though they pass every statistical test for randomness, because a short program computes them. A million bits straight from a quantum random-number generator have complexity close to a million bits: almost surely there is no structure to exploit, and the shortest ``program'' is essentially the string quoted verbatim. A state of $10^{23}$ particles all in one corner of a box has low complexity (a short loop prints the repeated coordinates); a generic thermalized state has complexity near the full length of its encoding. The slogan: \emph{low entropy is compressibility}, a property of the individual state $x$, with no ensemble in sight.
...
One special case recurs throughout the demon analysis below.
...
This section pulls the previous ones together into a single picture: for an embedded agent, knowledge about the environment is a physical resource, and it is the same quantity as the agent's capacity to optimize the environment.
...
\subsection{Back to optimizers}
\label{sec:backtoflint}

Return to Section~\ref{sec:optsys}. An optimizer drives the system it acts on from a broad range of configurations into a narrow target set. What does the existence of such an optimizer imply, thermodynamically?

The funneling is coarse-grained entropy reduction in a subsystem, so the books must balance through some mixture of the three channels (Section~\ref{sec:threetypes}): waste exported to the surroundings (Type 1), records kept in memory (Type 2), or pre-existing correlation spent (Type 3). The main implication is a selection-theorem-like conclusion: if the funneling beats the blind baseline of the environment's own dynamics, then by the Touchette--Lloyd theorem (Theorem~\ref{thm:tl}, Appendix~\ref{sec:tl}) and its algorithmic form \eqref{eq:algTL}, the system must carry mutual information with what it steers, a necessary (though, by Appendix~\ref{sec:tlcaveats}, not sufficient) ingredient of a world model. So at least this much modeling is forced by the bookkeeping, not read in by an observer. This is the thermodynamic counterpart of the observer-independence argued in Section~\ref{sec:objective}: the presence of a world model, like the presence of an optimizer, is an objective fact about the system, certified by the entropy it removes rather than attributed by whoever is watching.

\section{Takeaways}
\label{sec:takeaways}

\begin{itemize}[leftmargin=2em]
  \item An optimizer is a physical thing whose presence drives the system it acts on, from a broad range of initial conditions and despite perturbations, into a narrow target: conditioning on the optimizer makes your uncertainty about where that system ends up small (it gives the system a convergent attractor). The amount of optimization is the entropy reduction it produces in that system, the bits by which it narrows the system's possibilities. Calling something an optimizer requires no claim about its internals or goals, only about its effect on the system it steers.
  \item Even an observer who cares only about prediction has an objective, information-theoretic reason to single out optimizers: the target is a short, robust description of where the system ends up (it converges to the target), so it forecasts the macroscopic outcome for a tiny fraction of the bits that the initial-condition route would require, and without knowing the initial conditions at all. This makes optimization an observer-independent feature of the world, not a stance projected by a particular observer, and answers the worry that intelligence is merely task-relative.
...
This appendix develops, as a self-contained worked example, the toy thermodynamic world referenced from two points elsewhere in these notes (the blind baseline of Appendix~\ref{sec:tl} and the Type-1 channel of Section~\ref{sec:threetypes}). It can be read at any point after Section~\ref{sec:secondlaw}. It follows Wentworth's essay \emph{Generalized Heat Engine}, whose goal is to take the distinctively ``thermo-flavored'' ideas of statistical mechanics (heat, work, engines, the impossibility of perpetual motion) and rebuild them in a setting with no physics at all, so that it becomes clear which parts of thermodynamics are really facts about information. The model also makes vivid a viewpoint, the \emph{designer's viewpoint}, that transfers directly to thinking about agents embedded in environments they cannot fully observe.
...
In Section~\ref{sec:threetypes} we saw that an agent can steer a subsystem by spending mutual information it already shares with it (the Type-3 channel). This appendix quantifies that channel: how much entropy reduction does each bit of mutual information actually buy? The answer connects optimization to \emph{modeling}.
...
One might naively conjecture that any entropy reduction at all requires sight, but this is false, and seeing why sharpens the eventual theorem. The dynamics alone can reduce entropy: a contracting dynamics (one that funnels many initial states toward the same point on its own, like a ball settling to the bottom of a valley against friction) funnels states all by itself, whatever the action; and the coin engine of Appendix~\ref{sec:twobaths} reduced the entropy of designated coins while completely blind. The right question is therefore not whether a policy reduces entropy, but whether it reduces entropy \emph{more than blindness allows}. Accordingly, define the \emph{blind baseline}
\[
...
\begin{itemize}[leftmargin=2em]
  \item A.~Flint, \emph{The ground of optimization}, AI Alignment Forum, 2020. A complementary descriptive treatment of optimization as \emph{optimizing systems} (a broad basin of attraction, a narrow target set, and robustness to perturbation), with a finer comparison of such systems along axes of robustness, duality, and retargetability. Recommended further reading on the behavioral characterization of Section~\ref{sec:optsys}.
  \item E.~Yudkowsky, \emph{Measuring optimization power}, LessWrong, 2008. A complementary quantitative proposal, measuring optimization by the improbability of the achieved outcome under random rearrangement; further reading alongside the entropy-reduction measure of Section~\ref{sec:optentropy}.
  \item D.~H.~Wolpert and W.~G.~Macready, \emph{No free lunch theorems for optimization}, IEEE Transactions on Evolutionary Computation 1(1):67--82, 1997; and D.~C.~Dennett, \emph{The Intentional Stance}, MIT Press, 1987. The two articulations of the observer-relative view of intelligence and agency that Section~\ref{sec:objective} states and answers: no optimizer is universally competent (competence is relative to a problem class), and agency is a predictive stance an observer adopts rather than an intrinsic property.
  ...
  \item Daniel~C and A.~Ebtekar, \emph{Algorithmic thermodynamics and three types of optimization}, AI Alignment Forum, 2025. The central organizing source for these notes: the characterization of optimizers by the convergent attractors they create and the information-theoretic case for attending to them whatever one's goals (Section~\ref{sec:optsys}), the three types of optimization (Section~\ref{sec:threetypes}), the argument that entropy should objectively measure optimization capacity (Section~\ref{sec:subjective}), the algorithmic refinements of the three types, and the embedded-agency synthesis (Section~\ref{sec:embedded}).
