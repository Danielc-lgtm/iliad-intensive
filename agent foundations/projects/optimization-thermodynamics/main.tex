...
\paragraph{Making the link precise.} The claim that ``encoding a good model'' is the same thing as ``having high algorithmic mutual information'' can be made exact through Shannon coding. Suppose the agent's memory $a$ encodes a computable probability distribution $q$ over environment states -- its belief about the environment, in the ordinary Bayesian sense (formally, $q$ is recovered from $a$ by a fixed program of size $O(1)$). One way to describe the true state $s$ \emph{given} $a$ is to reconstruct $q$ and transmit the Shannon codeword of $s$ under $q$, whose length is $\lceil \log \tfrac{1}{q(s)} \rceil$ bits (Section~\ref{sec:coding}). The shortest description can only be shorter, so
\[
K(s \mid a) \;\plt\; \log \frac{1}{q(s)}.
\]
This is the pointwise bound that underlies Zurek's identity (Section~\ref{sec:Kjustified}), now read in the agent's favour. Substituting it into the definition of algorithmic mutual information bounds the agent's knowledge from below by the probability its belief places on the truth:
\[
I(a : s) \;\peq\; K(s) - K(s \mid a) \;\pgt\; K(s) - \log \frac{1}{q(s)}.
\]

Now follow what Bayesian updating does to this inequality. As the agent gathers evidence about the environment and updates, the probability $q(s)$ it assigns to the \emph{true} state rises; its codelength $\log\tfrac{1}{q(s)}$ shrinks; the residual conditional complexity $K(s \mid a)$ shrinks with it; and the lower bound on $I(a:s)$ rises in step. In the limit of certainty, with $q$ concentrated on $s$, the codelength vanishes, $K(s \mid a) \peq 0$, and $I(a:s) \peq K(s)$: the agent's memory then shortens the description of the environment by the whole of $K(s)$. Updating a belief toward the truth and accumulating physically encoded mutual information are one process, seen two ways.

This is exactly the resource that buys optimization, which closes the loop with the rest of the notes. The algorithmic Touchette--Lloyd bound \eqref{eq:algTL} caps the complexity the agent can squeeze out of the environment at $I(a:s)$, attainably; so the optimization the agent can perform is at least
\[
K(s) - K(s \mid a) \;\pgt\; K(s) - \log \frac{1}{q(s)},
\]
and the environment can be driven down to a residual complexity of about $\log\tfrac{1}{q(s)}$, its entropy \emph{as the agent's belief finds it}. Concentrating more probability on the truth lowers that residual and raises the steering available. This is the individual-state, algorithmic form of the Touchette--Lloyd lesson of Section~\ref{sec:tl} -- more mutual information, more optimization -- and the rigorous content of the Type-3 channel of Section~\ref{sec:threetypes}, in which the agent funnels the environment precisely by spending $I(a:s)$.
