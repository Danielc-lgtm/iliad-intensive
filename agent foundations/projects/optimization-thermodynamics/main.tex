...
The funneling is coarse-grained entropy reduction in a subsystem, so the books must balance through some mixture of the three channels (export to the surroundings, records in memory, pre-existing correlation spent). The consequential entailment is the one we have been building toward: if the funneling exceeds the blind baseline of the environment's dynamics, then by Theorem~\ref{thm:tl} and its algorithmic sharpening \eqref{eq:algTL} the system \emph{must} carry mutual information with what it steers -- a necessary (though, by Section~\ref{sec:tlcaveats}, not sufficient) ingredient of a world model. At least this much modeling is forced by the bookkeeping rather than read in by an observer, which is the thermodynamic seed of the selection-theorems program. Flint's three axes then read thermodynamically: \emph{robustness} is the size of the entropy-disposal capacity (the environment it can dump waste into, its free memory, its stock of correlation); \emph{duality} is whether that bookkeeping is localized in an identifiable part (which is also why such a part is brittle); and \emph{retargetability} is whether the target is held as a compact, rewritable record.
