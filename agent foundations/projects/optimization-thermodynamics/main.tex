...
The cleanest intuitions come from control theory, and the thermostat is the standard picture. Whatever temperature the room starts at, and whatever disturbances arrive while the controller runs (a window opened, a cold night, a door left ajar), the room is driven back toward the set temperature. In the language of dynamical systems, the target is a \emph{convergent attractor}: a region the system is pulled into from far away and returns to after being pushed off it.

Two features carry all the weight, and both are visible in the thermostat.

The first is a \emph{broad range of initial conditions}. The optimizer does not need to begin from one special configuration; a wide variety of starting points are all funnelled to the same place. An observer who knows only that the room ``started somewhere'' is highly uncertain about its initial temperature, yet can be confident about where it ends up.

The second is \emph{robustness to perturbation}. The convergence survives disturbances applied while the process is underway, and this is what separates genuine optimization from a coincidence. A satellite coasting in orbit traces a precise path, but nudge it and it follows a different path forever: there is no target it reliably returns to, so it optimizes nothing. A ball rolling to the bottom of a valley against friction \emph{is} a (weak) optimizer: displace it and it rolls back. The presence or absence of a restoring tendency is the whole difference.

Two omissions are worth flagging, because the rest of the document depends on having dropped them. The characterization says nothing about an ``optimizer'' acting on a separate ``optimized'' thing: the relevant object is the whole system, and we ask only whether the whole funnels its configurations toward a target. And it says nothing about goals, preferences, or representations stored in a mind: a thermostat, a feedback circuit, a growing organism, a running optimization algorithm, and a team building a house are optimizers in exactly the same sense, each a physical process that drives a broad set of possibilities into a narrow target and defends it against perturbation. What unifies them is a structural fact about their dynamics, not a mental attribute. The next three subsections turn this behavioral picture into a number, explain why even an observer with no stake in the target should care about it, and argue that optimizer-hood is therefore an objective feature of the world.
...
Second, \emph{a tiny amount of information predicts the outcome}. All you need to learn is that an optimizer is present and what it is steering toward, that is, the target $T$. Write $O$ for this proposition (``an optimizer with target $T$ is operating on this subsystem''). Learning $O$ collapses your uncertainty about $Y$ from its no-optimizer value (call it $H_0$, which is of order $H(X)$) down to the residual uncertainty about \emph{which} configuration within $T$ is reached, $H(Y \mid O)$, which is small because $T$ is small. The bits of future prediction you gain by learning $O$ are
\[
I(O; Y) \;=\; H_0 - H(Y \mid O),
\]
a large number. The bits of present information you spent to gain them are just the description length of $O$ itself, essentially the cost of naming the target $T$, which is small. The whole point is the ratio of these two: a compact specification of a target buys a large reduction of uncertainty about a high-dimensional outcome. Per bit learned now, you predict many bits of the future. This is the exact inverse of the no-optimizer case, where you must pay $H(X)$ bits (the entire initial condition) for a forecast that only decays.

The thermostat makes the two routes concrete. To predict the room's temperature in an hour you could measure the position and velocity of every air molecule and integrate the equations of motion: astronomically many bits, and hopeless anyway because the molecular dynamics are chaotic. Or you could learn the single fact ``a thermostat set to $21^\circ\mathrm{C}$ is running'': a handful of bits, from which you predict $21^\circ\mathrm{C}$ with confidence. The second route spends almost nothing and predicts almost perfectly, without measuring a single molecule.

Robustness sharpens the conclusion. Because the optimizer reaches $T$ in spite of perturbations, your forecast ``the system ends in $T$'' holds even under disturbances you could not have foreseen or measured. You are therefore spared not only the cost of the initial condition but also the cost of modelling the perturbations: the optimizer absorbs both. An agent that cares only about prediction should, for this reason alone, scan its environment for optimizers and model them as optimizers, tracking what they steer toward, because nothing else returns as much forecast per bit invested.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Even an observer who cares only about prediction has an objective, information-theoretic reason to single out optimizers: learning the small fact that an optimizer with a given target is present predicts a large amount about the future (the system converges to the target) without knowing the initial conditions, at a forecast-per-bit ratio that ordinary initial-condition-based prediction cannot match. This makes optimization an observer-independent feature of the world, not a stance projected by a particular observer, and answers the worry that intelligence is merely task-relative.
