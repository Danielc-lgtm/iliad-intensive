...
\frame{\frametitle{Functional decision theory: intervene on the function}
	\begin{block}{The three rules side by side (Yudkowsky \& Soares 2017)}
		{\tiny
		\begin{itemize}
			\item Write $V$ for the outcome's value, $\mathrm{Act}$ for the physical-action node, and $\mathrm{fdt}(P,G,x)$ for the \emph{logical node} representing the output of the agent's decision function on observation $x$:
			$$\mathrm{EDT}(P,x)=\arg\max_a \mathbb{E}[V\mid \mathrm{Obs}{=}x,\ \mathrm{Act}{=}a],$$
			$$\mathrm{CDT}(P,G,x)=\arg\max_a \mathbb{E}[V\mid \mathrm{do}(\mathrm{Act}{=}a),\ \mathrm{Obs}{=}x],$$
			$$\boxed{\ \mathrm{FDT}(P,G,x)=\arg\max_a \mathbb{E}\big[V\mid \mathrm{do}\big(\mathrm{fdt}(P,G,x){=}a\big)\big]\ }$$
			\item The only change from CDT is \emph{which node you intervene on}. CDT intervenes on the physical act -- Peirce's \textbf{token} (``\emph{my} action is $a$''). FDT intervenes on the \textbf{type} -- the abstract function -- so every predictor, copy, or simulation computing that same function updates with it.
			\item \textbf{Newcomb:} setting $\mathrm{fdt}{=}$one-box also moves $\Omega$'s earlier prediction $\Rightarrow$ box full; FDT one-boxes. \textbf{Smoking lesion:} the lesion does not compute your decision, so $\mathrm{do}(\mathrm{fdt}{=}\text{smoke})$ leaves it untouched; FDT smokes. \textbf{FDT gets both right.}
			\item Caveat: the function's output is fixed of \emph{logical necessity}, so FDT must evaluate \emph{counterpossibles} (antecedents that run counter-to-logic) -- still the main open problem (cf.\ the 5-and-10 problem).
		\end{itemize}
		}
	\end{block}
}
...
\frame{\frametitle{The 5-and-10 problem}
	\begin{block}{The simplest decision, made hard by self-knowledge (Demski \& Garrabrant)}
		{\tiny
		\begin{itemize}
			\item You may take \$5 or \$10; utility $=$ the dollars. A child takes \$10. Can a fully self-aware embedded agent?
			\item Frame it for an embedded agent. The world is a deterministic program that pays out whatever number you take; the agent has \emph{its own source code} and the world's. It chooses by \textbf{proof search}: look for proofs of ``$(A{=}\$5)\Rightarrow U{=}x$'' and ``$(A{=}\$10)\Rightarrow U{=}y$'', and take the action with the higher \emph{proven} payoff.
			\item \textbf{The deep issue.} ``If you can know your own behaviour, it becomes hard to reason about what would happen if you behaved \emph{differently}.'' Because the agent is deterministic it takes exactly one action; the other is ruled out \emph{by logic}, so ``what if I did otherwise?'' is a counterfactual with an \textbf{impossible antecedent}.
			\item \textbf{The trap, concretely.} Suppose the agent can prove it takes \$5. Then $A\neq\$10$, so the material conditional ``$(A{=}\$10)\Rightarrow(U{=}\$0)$'' is \textbf{vacuously true} (a false antecedent implies anything). The agent proves it, ``concludes'' \$10 is worthless, and takes \$5 -- \emph{confirming} the proof. A self-fulfilling \textbf{spurious counterfactual}.
			\item The fault is not bad logic but \emph{too much self-knowledge}: the action is a fixed fact, and a naive counterfactual over a fixed fact is vacuous.
		\end{itemize}
		}
	\end{block}
}

\frame{\frametitle{Spurious proofs and L\"ob's theorem}
	\begin{block}{Why a naive proof-searcher is genuinely unsafe}
		{\tiny
		\begin{itemize}
			\item \textbf{L\"ob's theorem (formula + intuition).} Write $\Box P$ for ``$P$ is provable''. Then
			$$\text{if } \vdash (\Box P \to P)\ \text{ then } \vdash P; \qquad\text{equivalently}\qquad \vdash \Box(\Box P \to P)\to\Box P.$$
			Intuition: a system may treat ``provable $\Rightarrow$ true'' as safe \emph{only for what it already proves} -- so strongly that it cannot even prove its own consistency ($\Box\bot\to\bot$, i.e.\ G\"odel~II).
			\item \textbf{The spurious proof is available.} Let $S := \big((A{=}\$10)\Rightarrow(U{=}\$0)\big)$. The agent is built so that \emph{proving} $S$ makes it take \$5; then $A\neq\$10$, so $S$ holds vacuously, giving $\vdash(\Box S\to S)$. By L\"ob, $\vdash S$. A naive proof search can therefore prove the bad statement and take \$5.
			\item \textbf{Moral.} Ruling out spurious proofs needs extra structure (e.g.\ ``playing chicken with the universe''); a general account of \emph{logical counterfactuals} (counterpossibles) is open. The same L\"obian obstacle blocks an agent from fully trusting a successor's proofs -- a reflective-stability problem.
		\end{itemize}
		}
	\end{block}
}
